...
const DEFAULT_VALUES = {
  ...
  round_trip: false,
  vehicle_type: undefined as "daily_drive" | "large_loadable" | "oversized" | undefined,
}
...
export function extractParameters(url: URL): TripParamsUrl | null {
  ...
  if (!result.origin_lat || !result.origin_lng || !result.dest_lat || !result.dest_lng) {
    return null
  }
...
export function buildSearchParams(params: Partial<TripParamsUrl>): URLSearchParams {
  ...
  if (params.electric_vehicle !== undefined && params.electric_vehicle !== DEFAULT_VALUES.electric_vehicle) {
    searchParams.set("electric_vehicle", params.electric_vehicle ? "1" : "0")
  }
  if (params.round_trip !== undefined && params.round_trip !== DEFAULT_VALUES.round_trip) {
    searchParams.set("round_trip", params.round_trip ? "1" : "0")
  }
  if (params.vehicle_type !== undefined && params.vehicle_type !== DEFAULT_VALUES.vehicle_type) {
